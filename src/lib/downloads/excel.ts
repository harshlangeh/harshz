...
async function buildWorkbook(data: DownloadData) {
  ...
  const coverRows: (string | number)[][] = [
    ...
    ['Occupancy',   `${data.projectInfo.occupancyFixed || '—'} fixed / ${data.projectInfo.occupancyFloating || '—'} floating / ${data.projectInfo.occupancyTotal || '—'} total`],
  ...
  const summaryRows: (string | number)[][] = [
    ...
    ['Occupancy — Total',     data.projectInfo.occupancyTotal    || '—'],
    ['Climate Zone',          data.projectInfo.climateZone    || '—'],
    ['Address',               [data.projectInfo.city, data.projectInfo.state, data.projectInfo.country].filter(Boolean).join(', ') || '—'],
    [],
    ['RATING SUMMARY'],
