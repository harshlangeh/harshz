...
const data = [
  ...
  {
    id: 3, title: "Energy Optimization", max: 18,
    criteria: [
  ...
  {
    id: 4, title: "Occupant Comfort", max: 12,
    criteria: [
  ...
  {
    id: 7, title: "Sustainable Building Materials", max: 12,
    criteria: [
  ...
  {
    id: 8, title: "Life Cycle Costing", max: 5,
    criteria: [
      { id: 22, name: "Life Cycle Cost Analysis", max: 5, type: "Optional" }
    ]
  },
  {
    id: 9, title: "Socio-Economic Strategies", max: 8,
    criteria: [
  ...
  {
    id: 10, title: "Performance Metering and Monitoring", max: 7,
    criteria: [
        ...
        <tr key={c.id} className={\`border-b transition-colors hover:bg-white-5 \${c.type === 'Mandatory' ? 'bg-dark-rose' : c.type === 'Partly Mandatory' ? 'bg-rose-10' : ''}\`}>
          ...
          <td className="py-3 px-4 text-center">
            <input 
              ...
              className="w-16 bg-black/20 border border-white/10 rounded px-2 py-1 text-center text-white focus:outline-none focus:border-green focus:ring-1 focus:ring-green"
              placeholder="0"
            />
          </td>
...
const dir = 'src/app/griha-v2019';
if (!fs.existsSync(dir)){
    fs.mkdirSync(dir, { recursive: true });
}
fs.writeFileSync(path.join(dir, 'page.tsx'), componentCode.trim());
console.log('GRIHA V2019 checklist generated.');
